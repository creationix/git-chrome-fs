...
// Restore an entry and register it in the entry cache.
function registerEntry(entry, callback) {
  if (!callback) return registerEntry.bind(null, entry);
  fileSystem.restoreEntry(entry, function (result) {
    if (result) {
      var rootPath = pathJoin(result.filesystem.name, result.fullPath);
...
function rename(oldPath, newPath, callback) {
  getDir(dirname(newPath), function (err, newParent) {
    if (!newParent) return callback(err);
    getEntry(oldPath, "getFile", {}, function(err, entry) {
      if (!entry) return callback(err);
      entry.moveTo(newParent, basename(newPath), onRenamed, onError);
    });
  });

  function onRenamed(entry) {
    delete entryCache[oldPath];
    entryCache[newPath] = entry;
    callback();
  }
